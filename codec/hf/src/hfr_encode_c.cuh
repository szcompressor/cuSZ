...
__global__ void KCU_HFR_encode(
    ...
    typename C::Hf* dn_bitstream, psz::_future::bheader<typename C::T, C::Radius>* dn_headers)
{
...
// Member function template (class params + method's Radius); UseScan unused here.
    ...
    psz::_future::bheader<T, Radius>* dn_headers, void* stream, RMerge rm, SMerge sm)
{
  ...
  dispatch_merge_host(rm, sm, [&](auto rm_tag, auto sm_tag) {
    ...
    phf::KCU_HFR_encode<C, RM, SM><<<nblock, nthread, 0, (cudaStream_t)stream>>>(
        in_eq, len, runtime_book, dn_bitstream, dn_headers);
  });
...
// Function-template explicit instantiation (avoids ODR vs hfr.cu.inl's struct-level v1).
#define __INSTANTIATE_HFR_ENCODE(T, MAG, RED, RAD)                                         \
  template int                                                                             \
      phf::module::HFR_encoder<T, MAG, RED, false, uint32_t>::template GPU_kernel_v2<RAD>( \
          T*, size_t, uint32_t*, uint32_t*, psz::_future::bheader<T, RAD>*, void*, RMerge, \
          SMerge);

// 1-arg form: fan out u1/u2 at canonical MAG=10, RAD=128 (mirrors __INSTANTIATE_HFR_PBK{C,GO}_1).
#define __INSTANTIATE_HFR_ENCODE_1(RED)           \
  __INSTANTIATE_HFR_ENCODE(uint8_t, 10, RED, 128) \
  __INSTANTIATE_HFR_ENCODE(uint16_t, 10, RED, 128)
