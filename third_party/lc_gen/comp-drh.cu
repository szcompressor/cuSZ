...
//
#include "../lc/components/d_DIFF_4.h"
#include "../lc/components/d_HCLOG_2.h"
#include "../lc/components/d_RZE_2.h"
...
__global__ void d_reset_drh_comp() { g_chunk_counter = 0; }
        ...
        int* const __restrict__ outsize, int* const __restrict__ fullcarry)
{
  ...
  do {
    ...
    if (good) {
      ...
      in = out;
      out = tmp;
      good = d_DIFF_4(csize, in, out, temp);
      __syncthreads();
    }
    if (good) {
      byte* tmp = in;
      in = out;
      out = tmp;
      good = d_RZE_2(csize, in, out, temp);
      __syncthreads();
    }
    if (good) {
      byte* tmp = in;
      in = out;
      out = tmp;
      good = d_HCLOG_2(csize, in, out, temp);
      __syncthreads();
    }
