...
#define DESTROY_GPUEVENT_PAIR \
  cudaEventDestroy(a);        \
  cudaEventDestroy(b);
#define START_GPUEVENT_RECORDING(STREAM) cudaEventRecord(a, (cudaStream_t)STREAM);
#define STOP_GPUEVENT_RECORDING(STREAM)     \
  cudaEventRecord(b, (cudaStream_t)STREAM); \
  cudaEventSynchronize(b);
#define TIME_ELAPSED_GPUEVENT(PTR_MILLISEC) cudaEventElapsedTime(PTR_MILLISEC, a, b);
...
#define DESTROY_GPUEVENT_PAIR \
  hipEventDestroy(a);         \
  hipEventDestroy(b);
#define START_GPUEVENT_RECORDING(STREAM) hipEventRecord(a, (hipStream_t)STREAM);
#define STOP_GPUEVENT_RECORDING(STREAM)   \
  hipEventRecord(b, (hipStream_t)STREAM); \
  hipEventSynchronize(b);
#define TIME_ELAPSED_GPUEVENT(PTR_MILLISEC) hipEventElapsedTime(PTR_MILLISEC, a, b);

#elif defined(_PORTABLE_USE_1API)

#define SYCL_TIME_DELTA(EVENT, MILLISEC)                                                      \
  auto sycl_time_delta = [](sycl::event& e) {                                                 \
    cl_ulong start_time = e.get_profiling_info<sycl::info::event_profiling::command_start>(); \
    cl_ulong end_time = e.get_profiling_info<sycl::info::event_profiling::command_end>();     \
...
#define TIME_ELAPSED_GPUEVENT(PTR_MILLISEC)                                                   \
  *PTR_MILLISEC = (end->get_profiling_info<sycl::info::event_profiling::command_end>() -      \
