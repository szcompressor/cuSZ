...
class SpcodecVec {
 ...
 public:
  ...
  void encode(
      T* in, size_t const in_len, BYTE*& out, size_t& out_len,
      cudaStream_t stream = nullptr, bool dbg_print = false)
  {
    Header header;

    psz::spv_gather<T, M>(
        in, in_len, this->d_val, this->d_idx, &rte.nnz, &milliseconds, stream);
  ...
  void decode(BYTE* coded, T* decoded, cudaStream_t stream = nullptr)
  {
    Header header;
    CHECK_CUDA(cudaMemcpyAsync(
        &header, coded, sizeof(header), cudaMemcpyDeviceToHost, stream));

#define ACCESSOR(SYM, TYPE) \
  reinterpret_cast<TYPE*>(coded + header.entry[Header::SYM])
    auto d_idx = ACCESSOR(IDX, uint32_t);
    auto d_val = ACCESSOR(VAL, T);
#undef ACCESSOR

    psz::spv_scatter<T, M>(
        d_val, d_idx, header.nnz, decoded, &milliseconds, stream);
  }
  ...
  void subfile_collect(
      Header& header, size_t len, cudaStream_t stream, bool dbg_print)
  {
...
#undef PRINT_ENTRY
    ...
    CHECK_CUDA(cudaMemcpyAsync(
        d_spfmt, &header, sizeof(header), cudaMemcpyHostToDevice, stream));
