...
class Predictor {
 ...
 private:
  ...
  struct DerivedLengths {
    struct {
      ...
      void set_serialized()
      {
        serialized = ConfigHelper::get_serialized_len(len3);
      }
    } base, anchor, aligned;
 ...
 public:
  ...
  void derive_lengths(cusz_predictortype predictor, dim3 base)
  {
    this->__derive_len(base, this->rtlen);
  }

  void init(
      cusz_predictortype predictor, size_t x, size_t y, size_t z,
  ...
  void construct(
      cusz_predictortype predictor, dim3 const len3, T* data, T** ptr_anchor,
      E** ptr_errctrl, T** ptr_outlier, double const eb, int const radius,
      cudaStream_t stream)
  {
  ...
  void reconstruct(
      cusz_predictortype predictor, dim3 len3, T* outlier_xdata, T* anchor,
      E* errctrl, double const eb, int const radius, cudaStream_t stream)
  {
    // derive_lengths(LorenzoI, len3);
