...
void pszctx_parse_control_string(cusz_context* ctx, const char* in_str, bool dbg_print)
{
    ...
    auto optmatch   = [&](std::vector<std::string> vs) -> bool { return psz_utils::check_opt_in_list(k, vs); };
    ...
    for (auto kv : opts) {
        ...
        // else if (optmatch({"anchor"}) and is_enabled(v)) { ctx->use_anchor = true; }
...
void pszctx_parse_argv(cusz_context* ctx, int const argc, char** const argv)
{
    ...
    std::string opt;
    auto        optmatch = [&](std::vector<std::string> vs) -> bool { return psz_utils::check_opt_in_list(opt, vs); };
...
void pszctx_load_demo_datasize(cusz_context* ctx, void* name)
{
...
// cusz_context::cusz_context(int argc, char** const argv)
// {
//     pszctx_parse_argv(this, argc, argv);
//     pszctx_validate(this);
// }

// cusz_context::cusz_context(const char* in_str, bool dbg_print) { pszctx_set_field_from_str(this, in_str, dbg_print);
// }
...
void pszctx_set_huffbyte(cusz_context* ctx, int _)
{
    ...
    // ctx->codecs_in_use  = codec_force_fallback() ? 0b11 /*use both*/ : 0b01 /*use 4-byte*/;
}
