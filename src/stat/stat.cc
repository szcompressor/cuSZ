...
template <typename T>
cusz_error_status histogram_ser(T* in, size_t const inlen, uint32_t* out_hist, int const outlen, float* milliseconds)
{
    auto t1 = hires::now();
...
#define SPECIALIZE_HIST_SER(T)                                                                             \
    ...
    {                                                                                                      \
        return psz::detail::histogram_ser<T>(in, inlen, out_hist, nbin, milliseconds);                     \
    }
