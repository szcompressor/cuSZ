...
void GPU_extrema_rawptr(T* d_ptr, size_t len, T res[4])
{
  ...
  auto minel = std::min_element(oneapi::dpl::execution::seq, g_ptr, g_ptr + len) - g_ptr;
  auto maxel = std::max_element(oneapi::dpl::execution::seq, g_ptr, g_ptr + len) - g_ptr;
  res[MINVAL] = *(g_ptr + minel);
  res[MAXVAL] = *(g_ptr + maxel);
  res[RNG] = res[MAXVAL] - res[MINVAL];

  auto sum = std::reduce(oneapi::dpl::execution::seq, g_ptr, g_ptr + len, (T)0.0, std::plus<T>());
  res[AVGVAL] = sum / len;
}
