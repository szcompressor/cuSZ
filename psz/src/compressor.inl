...
template <typename T, typename E>
void* CP<T, E>::compress_init(psz_context* ctx)
{
  constexpr auto iscompression = true;
...
template <typename T, typename E>
void* CP<T, E>::decompress_init(psz_header* header)
{
  constexpr auto iscompression = false;
...
template <typename T, typename E>
void CP<T, E>::compress(pszctx* ctx, PSZ_BUF* mem, T* in, u1** out, size_t* outlen, void* stream)
{
...
template <typename T, typename E>
void CP<T, E>::decompress(psz_header* header, PSZ_BUF* mem, u1* in, T* out, psz_stream_t stream)
{
  using M = typename PSZ_BUF::M;
...
template <typename T, typename E>
void CP<T, E>::release(PSZ_BUF* mem)
{
  if (mem) delete mem;
}

template <typename T, typename E>
void CP<T, E>::compress_dump_internal_buf(pszctx* ctx, PSZ_BUF* mem, psz_stream_t stream)
{
  auto dump_name = [&](string t, string suffix = ".quant") -> string {
    return string(ctx->cli->file_input)                                                //
           + "." + string(ctx->cli->char_mode) + "_" + string(ctx->cli->char_meta_eb)  //
