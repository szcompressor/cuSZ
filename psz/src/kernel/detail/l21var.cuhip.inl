...
__forceinline__ __device__ void pred1d(
    Data thread_scope[SEQ], volatile bool* shmem_signum,
    volatile ErrCtrl* shmem_delta, Data from_last_stripe = 0)
{
...
__forceinline__ __device__ void write1d(
    volatile bool* shmem_signum, bool* signum, unsigned int dimx,
    unsigned int id_base, volatile ErrCtrl* shmem_delta = nullptr,
...
__forceinline__ __device__ void postquant_write2d(
    Data center[YSEQ + 1], ErrCtrl* delta, bool* signum, unsigned int dimx,
    unsigned int dimy, unsigned int stridey, unsigned int gix,
    unsigned int giy_base)
{
  /********************************************************************************
   * Depending on whether postquant is delayed in compression, deside
   *separating data-type signum and uint-type quantcode when writing to DRAM
   *(or not).
   ********************************************************************************/
  auto get_gid = [&](auto i) { return (giy_base + i) * stridey + gix; };
...
__global__ void c_lorenzo_1d1l(  //
    Data* data, ErrCtrl* delta, bool* signum, dim3 len3, dim3 stride3,
    FP ebx2_r)
{
  ...
  load1d<Data, FP, NTHREAD, SEQ>(
      data, len3.x, id_base, shmem.data, thread_scope, from_last_stripe,
      ebx2_r);

  /********************************************************************************
   * delta and signum
   ********************************************************************************/
  pred1d<Data, ErrCtrl, SEQ, true>(
      thread_scope, shmem.signum, shmem.delta, from_last_stripe);
  pred1d<Data, ErrCtrl, SEQ, false>(thread_scope, shmem.signum, shmem.delta);
  write1d<ErrCtrl, NTHREAD, SEQ>(
      shmem.signum, signum, len3.x, id_base, shmem.delta, delta);
}

template <
    typename Data = float, typename ErrCtrl = uint16_t, typename FP = float>
__global__ void c_lorenzo_2d1l_16x16data_mapto16x2(
    Data* data,      // input
    ErrCtrl* delta,  // output
    bool* signum,    // output
    dim3 len3, dim3 stride3, FP ebx2_r)
{
...
template <
    typename Data = float, typename ErrCtrl = uint16_t, typename FP = float,
    int BLOCK = 256, int SEQ = 8>
__global__ void x_lorenzo_1d1l(  //
    bool* signum, ErrCtrl* delta, Data* xdata, dim3 len3, dim3 stride3,
    FP ebx2)
{
  constexpr auto block_dim = BLOCK / SEQ;  // dividable

  // coalesce-load (warp-striped) and transpose in shmem (similar for store)
  typedef cub::BlockLoad<bool, block_dim, SEQ, cub::BLOCK_LOAD_WARP_TRANSPOSE>
      BlockLoadT_signum;
   ...
   *thread block
   ********************************************************************************/
  BlockLoadT_delta(temp_storage.load_delta)
      .Load(delta + (BIX * BDX) * SEQ, thread_scope_delta);
  __syncthreads();  // barrier for shmem reuse
  BlockLoadT_signum(temp_storage.load_signum)
      .Load(signum + (BIX * BDX) * SEQ, thread_scope.signum);
  __syncthreads();  // barrier for shmem reuse
...
#pragma unroll
  for (auto i = 0; i < SEQ; i++) {
    ...
    thread_scope.xdata[i] = id < len3.x  //
                                ? (thread_scope.signum[i] ? -1 : 1) *
                                      static_cast<Data>(thread_scope_delta[i])
                                : 0;
  }
  __syncthreads();
...
#pragma unroll
  for (auto i = 0; i < YSEQ; i++) {
    ...
    if (gix < len3.x and giy_base + i < len3.y)
      thread_scope[i] =
          (signum[gid] ? -1 : 1) * static_cast<Data>(delta[gid]);  // fuse
    else
