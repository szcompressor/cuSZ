...
__global__ void c_spline3d_infprecis_32x8x8data(
    ...
    CompactNum cn, FP eb_r, FP ebx2, int radius);
...
__device__ void spline3d_layout2_interpolate(
    volatile T1 s_data[9][9][33], volatile T2 s_ectrl[9][9][33], FP eb_r, FP ebx2, int radius);
}  // namespace device_api
...
__device__ void global2shmem_fuse(
    E* ectrl, dim3 ectrl_size, dim3 ectrl_leap, T* scattered_outlier, volatile T s_ectrl[9][9][33])
{
  constexpr auto TOTAL = 33 * 9 * 9;
...
__device__ void shmem2global_32x8x8data_with_compaction(
    volatile T1 s_buf[9][9][33], T2* dram_buf, DIM3 buf_size, STRIDE3 buf_leap, int radius,
...
__forceinline__ __device__ void interpolate_stage(
    volatile T1 s_data[9][9][33], volatile T2 s_ectrl[9][9][33], DIM3 data_size, LAMBDAX xmap,
  ...
  auto run = [&](auto x, auto y, auto z) {
    if (xyz33x9x9_predicate<BORDER_INCLUSIVE>(x, y, z, data_size)) {
      ...
      if CONSTEXPR (WORKFLOW == SPLINE3_COMPR) {
        ...
        //     (BLUE),CONSTEXPR (YELLOW),CONSTEXPR
  ...
  */
  ...
  if (reverse[2]) {
    interpolate_stage<
        ...
        s_data, s_ectrl, data_size, xhollow_reverse, yhollow_reverse, zhollow_reverse, unit,
        cur_eb_r, cur_ebx2, radius, interpolators[2]);
    ...
    interpolate_stage<
        ...
        s_data, s_ectrl, data_size, xyellow_reverse, yyellow_reverse, zyellow_reverse, unit,
        cur_eb_r, cur_ebx2, radius, interpolators[2]);
    interpolate_stage<
        ...
        s_data, s_ectrl, data_size, xblue_reverse, yblue_reverse, zblue_reverse, unit, cur_eb_r,
    ...
    interpolate_stage<
        ...
        s_data, s_ectrl, data_size, xyellow, yyellow, zyellow, unit, cur_eb_r, cur_ebx2, radius,
        interpolators[2]);
    // if(BIX==0 and BIY==0 and BIZ==0)
    //  printf("lv3s2\n");
    interpolate_stage<
        ...
        s_data, s_ectrl, data_size, xhollow, yhollow, zhollow, unit, cur_eb_r, cur_ebx2, radius,
        interpolators[2]);
  }
  // if(BIX==0 and BIY==0 and BIZ==0)
  // printf("lv3\n");
  ...
  if (reverse[1]) {
    interpolate_stage<
        ...
        s_data, s_ectrl, data_size, xhollow_reverse, yhollow_reverse, zhollow_reverse, unit,
        cur_eb_r, cur_ebx2, radius, interpolators[1]);
    interpolate_stage<
        T1, T2, FP, decltype(xyellow_reverse), decltype(yyellow_reverse),
        decltype(zyellow_reverse),  //
  ...
  if (reverse[0]) {
    ...
    interpolate_stage<
        ...
        s_data, s_ectrl, data_size, xhollow_reverse, yhollow_reverse, zhollow_reverse, unit,
        cur_eb_r, cur_ebx2, radius, interpolators[0]);
    interpolate_stage<
        T1, T2, FP, decltype(xyellow_reverse), decltype(yyellow_reverse),
        decltype(zyellow_reverse),  //
...
template <
    typename TITER, typename EITER, typename FP, int LINEAR_BLOCK_SIZE, typename CompactVal,
    typename CompactIdx, typename CompactNum>
    ...
    CompactIdx compact_idx, CompactNum compact_num, FP eb_r, FP ebx2, int radius)
{
...

  {
    ...
    shmem2global_32x8x8data_with_compaction<T, E, LINEAR_BLOCK_SIZE>(
        shmem.ectrl, ectrl, ectrl_size, ectrl_leap, radius, compact_val, compact_idx, compact_num);

    // shmem2global_32x8x8data<T, E, LINEAR_BLOCK_SIZE>(shmem.ectrl, ectrl, ectrl_size,
    // ectrl_leap);
