...
__global__ void KCU_prototype_x_lorenzo_1d1l(
...
__global__ void KCU_prototype_x_lorenzo_2d1l(
    Eq* const in_eq, T* const in_outlier, T* const out_data, dim3 const data_len3,
    dim3 const data_leap3, uint16_t const radius, Fp const ebx2)
{
  SETUP_ND_GPU_CUDA;
...
__global__ void KCU_prototype_x_lorenzo_3d1l(
    Eq* const in_eq, T* const in_outlier, T* const out_data, dim3 const data_len3,
    dim3 const data_leap3, uint16_t const radius, Fp const ebx2)
{
  SETUP_ND_GPU_CUDA;
    ...
    int const radius, void* stream)
{
  ...
  if (ndim() == 1) {
    ...
    psz::KCU_prototype_x_lorenzo_2d1l<T>
        <<<Grid2D, Block2D, 0, (GPU_BACKEND_SPECIFIC_STREAM)stream>>>(
            in_eq, in_outlier, out_data, data_len, data_leap3, radius, ebx2);
  }
  else if (ndim() == 3) {
    psz::KCU_prototype_x_lorenzo_3d1l<T>
