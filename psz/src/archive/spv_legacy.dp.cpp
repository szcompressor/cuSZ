...
void spv_gather_dpl(
    T* in, szt const in_len, T* d_val, M* d_idx, int* nnz, f4* milliseconds, dpct::queue_ptr q)
{
  ...
  *nnz = dpct::copy_if(
             oneapi::dpl::execution::make_device_policy(*q), zero, zero + in_len, in, d_idx,
             [](auto t) { return std::get<0>(t) != 0; }) -
         d_idx;
...
void spv_scatter_dpl(
    T* d_val, M* d_idx, int const nnz, T* decoded, f4* milliseconds, dpct::queue_ptr q)
{
  CREATE_GPUEVENT_PAIR;
  START_GPUEVENT_RECORDING(q);

  dpct::scatter(
      oneapi::dpl::execution::make_device_policy(*q), d_val, d_val + nnz, d_idx, decoded);
...
#define SPECIALIZE_SPV(T, M)                                                             \
  ...
  void psz::spv_gather<ONEAPI, T, M>(                                                    \
      T * in, szt const in_len, T* d_val, M* d_idx, int* nnz, f4* milliseconds, void* q) \
  {                                                                                      \
  ...
  void psz::spv_scatter<ONEAPI, T, M>(                                                   \
      T * d_val, M * d_idx, int const nnz, T* decoded, f4* milliseconds, void* q)        \
  {                                                                                      \
    psz::spv_scatter_dpl(d_val, d_idx, nnz, decoded, milliseconds, (dpct::queue_ptr)q);  \
  }
