...
void spv_gather_thrust(
    T* in, szt const in_len, T* d_val, M* d_idx, int* nnz, f4* milliseconds, cudaStream_t stream)
{
...
void spv_scatter_thrust(
    T* d_val, M* d_idx, int const nnz, T* decoded, f4* milliseconds, cudaStream_t stream)
{
  thrust::cuda::par.on(stream);
...
#define SPECIALIZE_SPV(T, M)                                                                    \
  ...
  void psz::spv_gather<CUDA, T, M>(                                                             \
      T * in, szt const in_len, T* d_val, M* d_idx, int* nnz, f4* milliseconds, void* stream)   \
  {                                                                                             \
    spv_gather_thrust<T, M>(in, in_len, d_val, d_idx, nnz, milliseconds, (cudaStream_t)stream); \
  }                                                                                             \
  template <>                                                                                   \
  void psz::spv_scatter<CUDA, T, M>(                                                            \
      T * d_val, M * d_idx, int const nnz, T* decoded, f4* milliseconds, void* stream)          \
  {                                                                                             \
    psz::spv_scatter_thrust(d_val, d_idx, nnz, decoded, milliseconds, (cudaStream_t)stream);    \
  }
