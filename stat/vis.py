...
def process_plot_3fig(ana_ori: AnalyzeSmoothnessWholeDatum,
    ...
    for i, (var, aux_line, median_line, title) in enumerate(
            zip([ana_ori.var_abs_1d, ana_qua.var_abs_1d, ana_qua.var_binary],
                [False, False, True],
                [False, True, True],
                [f"ori {name}: variance in mean-abs -- enc-dist", f"qua {name}: variance in mean-abs -- enc-dist",
                 f"qua {name}: binary variance -- enc-dist"]
                )):
...
def process_plot_5fig(ana_ori: AnalyzeSmoothnessWholeDatum,
    ...
    for i, (var, aux_line, median_line, title, cr_str) in enumerate(
            zip([
                ...
                [
                    f"ori {name}: 2Dvar in mean-abs -- enc-dist",
                    f"ori {name}: 1Dvar in mean-abs -- enc-dist",
                    f"qua {name}: 2Dvar in mean-abs -- enc-dist",
                    f"qua {name}: 1Dvar in mean-abs -- enc-dist",
                    None,
        ...
        if cr_str:
            ...
            ax[i].set_title("; ".join((title, aux_str)), fontsize=16)
        else:
            ax[i].set_title(title, fontsize=16)
        ax[i].legend()
