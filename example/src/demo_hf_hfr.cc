...
void TEST_using_codec(
    string fname_base, size_t const len, uint16_t const bklen)
{
  using FREQ = u4;
  ...
  codec_ref.encode(
      false, d_data, len, &encoded_ref, &encoded_len_ref, stream_ref);
  ...
  codec_hfr.encode(
      true, d_data, len, &encoded_hfr, &encoded_len_hfr, stream_hfr);
