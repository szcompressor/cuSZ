...
#define PREPARE_2                                                \
  ...
  auto pad_len = config.at("pad_len");                           \
                                                                 \
  auto h_in_data = make_unique_host<uint16_t>(len);              \
  auto d_in_data = make_unique_device<uint16_t>(pad_len);        \
  auto h_out_data = make_unique_host<uint16_t>(len);             \
  auto d_out_data = make_unique_device<uint16_t>(pad_len);       \
                                                                 \
...
int high_level_demo(char **argv)
{
  ...
  auto d_copy_out = make_unique_device<uint8_t>(archive_len);
  memcpy_allkinds<D2D>(d_copy_out.get(), d_archive, archive_len);

  codec->decode(
      d_copy_out.get(), archive_len, d_out_data.get(), data_len, stream);
  sync_by_stream(stream);
