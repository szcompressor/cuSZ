...
void gen_randint_array(T *a, SIZE len, int default_num, int ratio) {
  ...
  auto rand_position = [&]() {
    return std::experimental::randint(0ul, len - 1);
  };
  auto rand_signed_int = [&]() { return std::experimental::randint(-20, 20); };
  auto randomize = [&]() { // change 1/ratio
    for (auto i = 0; i < len / ratio; i++)
      a[rand_position()] += rand_signed_int();
  };
  // -----------------------------------------------------------------------------
  fill(), randomize();
}
...
void figure_out_eb(CAPSULE &data, double &eb, double &adjusted_eb,
                   bool use_r2r) {
  adjusted_eb = eb;
...
void exp__prepare_data(UNCOMPRESSED **uncompressed, UNCOMPRESSED **decompressed,
                       SIZE len, int base_number, int non_base_portion,
                       bool destructive,
  ...
  if (destructive)
    if (not uncompressed_backup)
      throw std::runtime_error("Destructive runtime must have data backed up.");
}
...
template <typename UNCOMPRESSED>
void verify(UNCOMPRESSED *&uncompressed, UNCOMPRESSED *&decompressed,
            SIZE len) {
  for (auto i = 0; i < len; i++) {
    auto un = uncompressed[i], de = decompressed[i];
    if (un != de) {
      printf("The decoding is corrupted, first not equal at %d: un[[ %d ]], "
             "de[[ %d ]]\n",
             i, (int)un, (int)de);
      return;
    }
  }
  printf("\nPASSED: The decoded/decompressed matches the "
         "original/uncompressed.\n");
}

template <typename DATA>
void verify_errorboundness(DATA *&origin, DATA *&reconstructed, double const eb,
                           SIZE len) {
  for (auto i = 0; i < len; i++) {
    auto un = (float)origin[i], de = (float)reconstructed[i];
    auto error = fabs(un - de);
    if (error >= eb) {
      printf("Errorboundness is not guaranteed, first seen at %d: origin[[ %f "
             "]], reconstructed[[ %f ]]\n",
             i, un, de);
      return;
    }
